...
class SaleRepository extends BaseRepository
{
    ...
    public function storeSale($input): Sale
    {
        try {
             ...
             * 1 point = point_earning_rate (default 50 Rs)
             */
            $earningRate = getSettingValue('point_earning_rate') ?: 50;
            $earnedPoints = $sale->grand_total / $earningRate;
            $customer->points = round($customer->points + $earnedPoints, 2);

            /**
             * ✅ Handle point redemption if applicable
             */
            if (!empty($input['point_discount']) && $input['point_discount'] > 0) {
                $redemptionRate = getSettingValue('point_redemption_rate') ?: 1;
                $pointsToDeduct = $input['point_discount'] / $redemptionRate;

                if ($customer->points < $pointsToDeduct) {
                    throw new UnprocessableEntityHttpException('Insufficient points for redemption.');
                }

                $customer->points = round($customer->points - $pointsToDeduct, 2);

                // Record withdrawal
                CustomerWithdrawal::create([
                    'customer_id' => $customer->id,
                    'points' => $pointsToDeduct,
                    'amount' => $input['point_discount'],
    ...
    public function updateSaleCalculation($input, $id)
    {
        ...
        $input['grand_total'] += $input['shipping'];
        $input['grand_total'] -= $input['point_discount'] ?? 0;
