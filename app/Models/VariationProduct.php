...
class VariationProduct extends BaseModel implements JsonResourceful
{
    ...
    public function prepareAttributes(): array
    {
        return [
            ...
            'variation_name' => $this->relationLoaded('variation') ? ($this->variation->name ?? '') : ($this->variation->name ?? ''),
            'variation_type_name' => $this->relationLoaded('variationType') ? ($this->variationType->name ?? '') : ($this->variationType->name ?? ''),
        ];
    }
