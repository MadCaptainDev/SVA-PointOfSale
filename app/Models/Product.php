...
class Product extends BaseModel implements HasMedia, JsonResourceful
{
    ...
    public function prepareAttributes(): array
    {
        ...
        $fields = [
            ...
            'stock' => $this->stock,
            'product_unit_name' => $this->relationLoaded('productUnit') ? $this->productUnit->toArray() : $this->getProductUnitName(),
            'sale_unit_name' => $this->relationLoaded('saleUnit') ? $this->saleUnit->toArray() : $this->getSaleUnitName(),
            'in_stock' => $this->relationLoaded('stocks') ? $this->stocks->sum('quantity') : $this->inStock($this->id),
        ];
        ...
        if (!$minimal) {
            $fields = array_merge($fields, [
                ...
                'notes' => $this->notes,
                'images' => $this->relationLoaded('mainProduct') ? $this->mainProduct->image_url : [],
                'product_category_name' => $this->relationLoaded('productCategory') ? $this->productCategory->name : '',
                'brand_name' => $this->relationLoaded('brand') ? $this->brand->name : '',
                'barcode_image_url' => $this->barcode_image_url,
                'barcode_symbol' => $this->barcode_symbol,
                'created_at' => $this->created_at,
                'purchase_unit_name' => $this->relationLoaded('purchaseUnit') ? $this->purchaseUnit->toArray() : $this->getPurchaseUnitName(),
                'warehouse' => $this->relationLoaded('stocks') ? $this->stocks : ($this->warehouse($this->id) ?? ''),
                'barcode_url' => Storage::url('product_barcode/barcode-PR_' . $this->id . '.png'),
            ]);
        }

        if ($this->relationLoaded('variationProduct') && $this->variationProduct) {
