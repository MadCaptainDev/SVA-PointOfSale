...
export const posAllProductAction = () => async (dispatch) => {
    apiConfig
        .get(`products?page[size]=0&skip_included=1&minimal=1`)
        .then((response) => {
            dispatch({
...
export const posAllProduct =
    (warehouse, isLoading = true) =>
        async (dispatch) => {
            ...
            apiConfig
                .get(`products?page[size]=0&warehouse_id=${warehouse}&skip_included=1&minimal=1`)
                .then((response) => {
                    dispatch({
...
export const fetchBrandClickable =
    (brandId, categoryId, warehouse, isLoading = true) => async (dispatch) => {
        ...
        await apiConfig
            .get(
                `products?filter[brand_id]=${brandId ? brandId : ""
                }&filter[product_category_id]=${categoryId ? categoryId : ""
                }&page[size]=0&warehouse_id=${warehouse ? warehouse : ""}&skip_included=1&minimal=1`
            )
            .then((response) => {
                dispatch({
