      ...
      <main className="invoice-container" role="main" aria-label="Invoice">
                ...
                <tr key={index}>
                  ...
                  <td style={{ textAlign: "left", padding: "4px 0" }}>
                    {currencySymbolHandling(
                      allConfigData,
                      paymentPrint.frontSetting.value.currency_symbol,
                      productName.quantity * calculateProductCost(productName.net_unit_price)
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="tax-summary" aria-label="Tax Summary">
          ...
          <p><span>Subtotal:</span><span> {currencySymbolHandling(
            allConfigData,
            currency,
            (Number(paymentPrint.grandTotal) || 0) + (Number(paymentPrint.cartItemPrint?.point_discount) || 0)
          )}</span></p>
          ...
          <p><span>Grand Total:</span><span> <strong style={{ fontSize: "1.25rem" }}> {currencySymbolHandling(
            allConfigData,
            currency,
            ...
            <span>
              <strong>
                {(() => {
                  ...
                  // Since we set paymentPrint BEFORE the response, currentPoints is STALE.
