...
import PrintData from "../../frontend/components/printModal/PrintData2";
...
const SaleDetails = (props) => {
    ...
    // Update receipt data when saleDetails changes
    useEffect(() => {
        if (saleDetails) {
            setPaymentPrint({
                ...saleDetails,
                products: saleDetails.sale_items || [],
                cartItemPrint: saleDetails,
                grandTotal: saleDetails.grand_total,
                discount: saleDetails.discount || 0,
                taxAmount: saleDetails.tax_amount || 0,
                shipping: saleDetails.shipping || 0,
                customer_name: saleDetails.customer
                    ? {
                        label: saleDetails.customer.name,
                        phoneNumber: saleDetails.customer.phone,
                        points: saleDetails.customer.points || 0
                    }
                    : null,
                settings: allConfigData,        // 👈 inject settings here
                frontSetting: frontSetting,     // 👈 inject front settings here
            });
        }
    }, [saleDetails, allConfigData, frontSetting]);
        ...
        return (
            <div className="d-none">
                <PrintData
                    ref={componentRef}
                    paymentType={"Cash"}
                    allConfigData={allConfigData || {}}    // 👈 fallback empty object
                    updateProducts={paymentPrint || {}}    // 👈 fallback empty object
                />
            </div>
        );
    };
                                    ...
                                    <div className="table-responsive">
                                        <table className="table border">
                                            <tbody>
                                                ...
                                                {saleDetails && saleDetails.point_discount > 0 && (
                                                    <tr>
                                                        <td className="py-3">
                                                        ...
                                                        <td className="py-3">
                                                            {currencySymbolHandling(
                                                                ...
                                                                frontSetting
                                                                    .value
                                                                    .currency_symbol,
                                                                saleDetails.point_discount
                                                            )}
                                                            {" "} ({saleDetails.redeem_points} Points)
                                                        </td>
                                                    </tr>
                                                )}
                                                <tr>
